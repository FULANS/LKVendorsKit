...
static const char LKReplacementKey = '\0';
static const char LKCachedProperties = '\0';
...
-(NSString *)chekIsLegal{
    ...
    if(self.originalArray.count <= 0){
        ...
        if ([object isKindOfClass:[NSString class]]) {
            ...
            type = @"model";
            NSMutableArray *cachedProperties = objc_getAssociatedObject([object class], &LKCachedProperties);
            if (!cachedProperties) {
                ...
                objc_setAssociatedObject([object class], &LKCachedProperties, cachedProperties, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
            }
...
-(NSArray *)search{
    ...
    if (self.searchText.length>0&&![ChineseInclude isIncludeChineseInString:self.searchText]) {
        for (int i=0; i<self.originalArray.count; i++) {
            ...
            NSDictionary * replacementDic = objc_getAssociatedObject([self class], &LKReplacementKey);
            if(replacementDic){
...
+(void)setupReplacement:(LKReplacement)relacement{
    if (relacement && [relacement() isKindOfClass:[NSDictionary class]]) {
        objc_setAssociatedObject(self, &LKReplacementKey, relacement(), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    } else {
        objc_setAssociatedObject(self, &LKReplacementKey, nil, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
}
