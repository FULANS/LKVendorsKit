...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self isSelectedWithIndex:indexPath.row]) {
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([self isSelectedWithIndex:indexPath.row]) {
        ...
        if (_isMultiSelect){
            cell.backgroundColor = _tagAttribute.normalBackgroundColor;
            cell.titleLabel.textColor = _tagAttribute.textColor;
            [self removeSelectedIndexWithIndex:indexPath.row];
        }
    }
    else {
        if (_isMultiSelect) {
            cell.backgroundColor = _tagAttribute.selectedBackgroundColor;
            cell.titleLabel.textColor = _tagAttribute.selectedTextColor;
            [self addSelectedIndexWithIndex:indexPath.row];
            
        } else {
            [self removeAllSelectedIndex];
            [self addSelectedIndexWithIndex:indexPath.row];
            [self reloadData];
        }
    }
    ...
    if (_completion) {
        ...
        [self.selectedIndexs enumerateObjectsUsingBlock:^(NSNumber * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            [selectedTags addObject:self.tags[obj.integerValue]];
        }];
        _completion(selectedTags,indexPath.row);
    }
}
...
- (BOOL)isSelectedWithIndex:(NSInteger)targetIndex{

    __block BOOL result = NO;
    [self.selectedIndexs enumerateObjectsUsingBlock:^(NSNumber * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj integerValue] == targetIndex) {
            result = YES;
            *stop = YES;
        }
    }];
...
- (void)removeSelectedIndexWithIndex:(NSInteger)targetIndex{
    ...
    if (result) {
        [self.selectedIndexs removeObject:result];
    }
}

- (void)addSelectedIndexWithIndex:(NSInteger)targetIndex{
    [self.selectedIndexs addObject:@(targetIndex)];
}

- (void)removeAllSelectedIndex{
