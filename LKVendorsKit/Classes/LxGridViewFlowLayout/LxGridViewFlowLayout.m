...
- (void)setLongPressGestureRecognizerEnable:(BOOL)longPressGestureRecognizerEnable
{
    _longPressGestureRecognizer.enabled = longPressGestureRecognizerEnable;
}

- (BOOL)longPressGestureRecognizerEnable{
...
- (void)longPressGestureRecognizerTriggerd:(UILongPressGestureRecognizer *)longPress
{
    switch (longPress.state) {
        ...
        case UIGestureRecognizerStateBegan:
        {
            ...
            UICollectionViewCell *sourceCell = (UICollectionViewCell *)sourceCollectionViewCell;
            
            _beingMovedPromptView = [[UIView alloc]initWithFrame:CGRectOffset(sourceCollectionViewCell.frame, -10, -10)];
            
            CGRect oldframe = _beingMovedPromptView.frame;
            oldframe.size.width += 20;
            oldframe.size.height += 20;
            _beingMovedPromptView.frame = oldframe;
            
            sourceCollectionViewCell.highlighted = YES;
            UIView * highlightedSnapshotView = [self snapshotView:sourceCell];
            ...
            UIView * snapshotView = [self snapshotView:sourceCell];
...
- (UIView *)snapshotView:(UIView *)source{
    
    UIView *snapshotView = [[UIView alloc]init];
    
    UIView *cellSnapshotView = nil;
    
    if ([source respondsToSelector:@selector(snapshotViewAfterScreenUpdates:)]) {
        cellSnapshotView = [source snapshotViewAfterScreenUpdates:NO];
    } else {
        CGSize size = CGSizeMake(source.bounds.size.width + 20, source.bounds.size.height + 20);
        UIGraphicsBeginImageContextWithOptions(size, source.opaque, 0);
        [source.layer renderInContext:UIGraphicsGetCurrentContext()];
