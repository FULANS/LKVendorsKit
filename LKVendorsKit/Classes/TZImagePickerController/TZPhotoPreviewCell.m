...
- (void)setIsSocialStyle:(BOOL)isSocialStyle {
    _isSocialStyle = isSocialStyle;
    if (isSocialStyle) {
...
- (void)configPlayerWithItem:(AVPlayerItem *)playerItem {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(playerDidEndPlay) name:AVPlayerItemDidPlayToEndTimeNotification object:self.player.currentItem];
}
...
- (void)appDidBecomeActiveNotification {
    ...
    if (_player && _player.rate == 0.0 && self.isSocialStyle) {
        [self playVideo];
    }
}
