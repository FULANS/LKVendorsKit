...
- (void)doneButtonPressed:(UISwipeGestureRecognizer *)gesture{
...
- (void)longPressed:(UILongPressGestureRecognizer *)gesture {
    ...
    UIAlertController *controller = [UIAlertController alertControllerWithTitle:nil message:nil preferredStyle:preferredStyle];
...
- (void)configPageView {
    ...
    _pageLabel.backgroundColor = [UIColor colorWithWhite:0 alpha:0.6];
...
- (void)viewDidLayoutSubviews {
    ...
    _saveBtn.frame = CGRectMake(self.view.tz_width - 56, 10 + statusBarHeightInterval, 44, 44);
    ...
    _pageLabel.frame = CGRectMake(0, toolBarTop, self.view.tz_width, toolBarHeight);
    _pageLabel.hidden = !self.isShowPage;
}
...
- (void)collectionView:(UICollectionView *)collectionView willDisplayCell:(UICollectionViewCell *)cell forItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([cell isKindOfClass:[TZPhotoPreviewCell class]]) {
        ...
        TZVideoPreviewCell *videoCell = (TZVideoPreviewCell *)cell;
        if (videoCell.isSocialStyle) {
            if (!self.displayIndexPath) {
                [videoCell playVideo];
            } else if (self.displayIndexPath != indexPath) {
                [videoCell playVideo];
                [(TZVideoPreviewCell *)[collectionView cellForItemAtIndexPath:self.displayIndexPath] pauseVideo];
            }
        } else {
...
- (void)refreshNaviBarAndBottomBarState {
    _pageLabel.text = [NSString stringWithFormat:@"%zd/%zd", _currentIndex+1, _photos.count];
    id photo = _photos[_currentIndex];
...
-(void)saveImage{
   UICollectionViewCell *cell = _collectionView.visibleCells.firstObject;
    ...
    if ([cell isKindOfClass:[TZPhotoPreviewCell class]]) {
        TZPhotoPreviewCell * PreviewCell = (TZPhotoPreviewCell *)cell;
        image = PreviewCell.previewView.imageView.image;
        //保存图片到相册
        UIImageWriteToSavedPhotosAlbum(image, self, @selector(image:didFinishSavingWithError:contextInfo:), NULL);
    }
    else if([cell isKindOfClass:[TZVideoPreviewCell class]]){
        TZVideoPreviewCell * PreviewCell = (TZVideoPreviewCell *)cell;

        NSURL *videoURL = PreviewCell.videoURL;
        NSString *path = videoURL.path;
        if (PreviewCell.player) {
...
- (void)image: (UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    ...
    !self.saveButtonClickBlock ? : self.saveButtonClickBlock(isSuccess,msg);
    // [_doneButton setTitle:[NSBundle tz_localizedStringForKey:@"Done"] forState:UIControlStateNormal]; 使用库的国际化
