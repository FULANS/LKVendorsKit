...
- (void)tick:(CADisplayLink *)link
{
    ...
    [attText addAttribute:NSForegroundColorAttributeName value:[UIColor colorWithRed:0.28 green:0.35 blue:0.54 alpha:1.0] range:NSMakeRange(0, attText.length - 3)];
...
- (CGRect)updateFrame:(CGRect)frame point:(CGPoint)point
{
    ...
    BOOL condition2 = frame.origin.x + frame.size.width <= [UIScreen mainScreen].bounds.size.width;
    if (condition1 && condition2) {
        frame.origin.x += point.x;
    }
    // y值
    condition1 = frame.origin.y >= 20;
    condition2 = frame.origin.y + frame.size.height <= [UIScreen mainScreen].bounds.size.height;
    if (condition1 && condition2) {
